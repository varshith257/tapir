...
case class OpenTelemetryMetrics[F[_]](meter: Meter, metrics: List[Metric[F, _]]) {
  ...
  /**
   * Registers a `request_active{path, method}` up-down-counter (assuming
   * default labels).
   */
  ...
  /**
   * Registers a `request_total{path, method, status}` counter (assuming default
   * labels).
   */
  ...
  /**
   * Registers a `request_duration_seconds{path, method, status, phase}`
   * histogram (assuming default labels).
   */
  ...
  /**
   * The interceptor which can be added to a server's options, to enable metrics
   * collection.
   */
...
object OpenTelemetryMetrics {
   ...
   *   - `http.response.status_code` - HTTP response status code (200, 404,
   *     etc.).
   */
  lazy val OpenTelemetryAttributes: MetricLabels = MetricLabels(
    ...
    forResponse = List(
      ...
      "error.type" -> { case Left(ex) => ex.getClass.getSimpleName },
    ),
  )
  ...
  def apply[F[_]](otel: OpenTelemetry, metrics: List[Metric[F, _]]): OpenTelemetryMetrics[F] =
    apply(defaultMeter(otel), metrics)
   ...
   *
   * Status is by default the status code class (1xx, 2xx, etc.), and phase can
   * be either `headers` or `body` - request duration is measured separately up
   * to the point where the headers are determined, and then once again when the
   * whole response body is complete.
   */
  ...
  def default[F[_]](otel: OpenTelemetry, labels: MetricLabels): OpenTelemetryMetrics[F] =
    default(defaultMeter(otel), labels)
    ...
    Metric[F, DoubleHistogram](
      ...
      onRequest = (req, recorder, m) =>
        m.eval {
          ...
          EndpointMetric().onResponseHeaders { (ep, res) =>
            m.eval {
              val otLabels =
                merge(
  ...
  private def defaultMeter(otel: OpenTelemetry): Meter =
    otel.meterBuilder("tapir").setInstrumentationVersion("1.0.0").build()
